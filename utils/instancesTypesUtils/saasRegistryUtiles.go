...
func GetAllSubscriptions(uri string, token string) ([]map[string]interface{}, error) {
	...
	for nextPage {
		...
		req, err := http.NewRequest("GET", fmt.Sprintf("%s/saas-manager/v1/application/subscriptions?page=%d", uri, page), nil)
		if err != nil {
			return nil, err
		}
...
func UpdateSubscription(uri string, tenantId string, token string) error {
	resp, err := http.NewRequest("PATCH", fmt.Sprintf("%s/saas-manager/v1/application/tenants/%s/subscriptions", uri, tenantId), nil)
...
func GetSubscription(uri, tenantId, token string) (map[string]interface{}, error) {
	saasRegistryRes := struct {
		Subscriptions []map[string]interface{} `json:"subscriptions"`
	}{}
	resp, err := http.NewRequest("GET", fmt.Sprintf("%s/saas-manager/v1/application/subscriptions?tenantId=%s", uri, tenantId), nil)
	resp.Header.Add("Authorization", fmt.Sprintf("Bearer %s", token))
	if err != nil {
		return nil, err
	}
